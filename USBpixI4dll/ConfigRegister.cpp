  ...
  rawDataHistogrammers((MultiChipWithSingleBoard?MAX_CHIP_COUNT:1), RawDataHistogrammer(TOTHisto, ConfHisto, HitLV1Histo, LV1IDHisto, BCIDHisto, isFEI4B))
{
  ...
  for (int chip = 0; chip < rawDataHistogrammers.size(); chip++)
  {
...
{
  for (int chip = 0; chip < HitLV1Histo.size(); chip++)
  {
...
{
  for (int chip = 0; chip < LV1IDHisto.size(); chip++)
  {
    for (int LV1ID = 0; LV1ID < 4096; LV1ID++)
    {
...
{
  for (int chip = 0; chip < BCIDHisto.size(); chip++)
  {
    for (int BCID = 0; BCID < 8192; BCID++)
    {
...
void ConfigRegister::GetConfHisto(int col, int row, int confStep, int readout_channel, int &Value)
{
  if ((col < 80) && (col >= 0) 
      ...
      && (readout_channel >= 0) && (readout_channel < ConfHisto.size()))
  {
...
void ConfigRegister::ClearConfHisto()
{
	for (int col = 0; col < 80; col++)
	{
		...
		{
			for (int confStep = 0; confStep < 1024; confStep++)
			{
			  for (int chipid = 0; chipid < ConfHisto.size(); chipid++)
        {
...
void ConfigRegister::MakeConfHisto(int confStep)
{
  ...
  for (int col = 0; col < 80; col++)
  {
    for (int row = 0; row < 336; row++)
    {
      for (int chip_id = 0; chip_id < ConfHisto.size(); chip_id++)
      {
        int sram_addr = 0;
        sram_addr += col;
        sram_addr += row * 80;
...
void ConfigRegister::ClearTOTHisto()
{
  for (int col = 0; col < 80; col++)
  {
    for (int row = 0; row < 336; row++)
    {
      ...
      {
        for (int chip_id = 0; chip_id < TOTHisto.size(); chip_id++)
        {
...
{
  for (int chip = 0; chip < record_streams.size(); chip++)
  {
