...
unix {
   ...
   INCLUDEPATH += $${system(root-config --incdir)} $(DATA_VIEWER) $(PIX_LIB)  \
	          $(PIX_LIB)/PixConfDBInterface $(PIX_ANA) $(DBEDT) \
                  $(VME_INTERFACE) $(USB_INCL_DIR) $(USBPIXI4DLL) External
...
win32{
  ...
  INCLUDEPATH += . $(ROOTSYS)/include $(VME_INTERFACE) $(DATA_VIEWER) \ 
		 $(PIX_LIB) $(PIX_LIB)/PixConfDBInterface $(PIX_ANA) \
		 $(DBEDT) $(USB_INCL_DIR) $(USBPIXI4DLL) External
