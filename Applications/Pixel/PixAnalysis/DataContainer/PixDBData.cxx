...
TH2F* PixDBData::getDACMap(int in_chip, const char *in_type)
{
  ...
  int binx, biny, chip, col, row, chip_min=0, chip_max=m_nFe, nrows=m_nRows, ncols=m_nCols*m_nFe;
  if(m_nFe>3){
    nrows *= 2;
    ncols /=2;
  }
			...
			bool doCalib)
{
  ...
  for(int i=0;i<npars;i++){
    ...
    std::stringstream b;
    b<< i;
    m_parHistos.push_back(new Histo("par"+b.str(),m_fitClass->getFuncName(fittype)+" par. "+b.str(),m_nFe*m_nCols,-0.5,-0.5+(double)(m_nFe*m_nCols),
				    m_nRows,-0.5,-0.5+(double)m_nRows));
  }
  m_parHistos.push_back(new Histo("CHI2",m_fitClass->getFuncName(fittype)+" chi2",m_nFe*m_nCols,-0.5,-0.5+(double)(m_nFe*m_nCols),
				    m_nRows,-0.5,-0.5+(double)m_nRows));
  if(m_fitClass->hasVCAL(fittype)){
