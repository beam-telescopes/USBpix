...
std::string STEUDAQ::getProducerName()
{
	if (m_producer_id==0)
		...
		return (QString("USBpixI4-") + QString::number(m_producer_id)).toStdString();
}
...
bool STEUDAQ::start(QString RunControlAddress)
{
  if(STE_DEBUG) std::cout << "STEUDAQ: start producer" << std::endl;
	...
	try
	{
	  ...
	  m_producer -> StartCommandReceiver();
...
void STEUDAQ::initControler2() 
{
	...
	if(STE_DEBUG) std::cout << "EUDAQ: Modules configured, setting scan options now!" << std::endl;
