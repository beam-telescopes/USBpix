...
#ifdef WIN32
#include <stdlib.h>
//#include <../SiLibUSB/SiUSBDeviceManager.h>
#else
...
void STRodCrate::clearVme(){
  ...
  if(m_USBinit){
    // deletion automatically calls libusb_exit
#ifndef WIN32
    delete USBDevMan;
    USBDevMan = 0;
#endif
...
void STRodCrate::loadModuleGroup( DBInquire *grpInq ){
  ...
  try{
    ...
    USBI3PixController *u3pc = dynamic_cast<USBI3PixController*>(pc);
#ifndef NOTDAQ
    RodPixController *rpc = dynamic_cast<RodPixController*>(pc);
    if(rpc!=0){
      if(m_IFtype!=tRCCVME){
...
{ 
  if(iGrp>=0 && iGrp<(int)m_pixModuleGroups.size()){
    ...
    if(pc!=0){
 	...
 	return 1;
#endif
      //      if(dynamic_cast<USBPixController *>(pc)!=0 || dynamic_cast<USBI3PixController *>(pc)!=0)
