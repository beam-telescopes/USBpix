...
void STPixModuleGroup::CtrlThread::scan()
{
  ...
  try{  
	...
	while(cfg.loop(1) && !getSTPixModuleGroup()->doAbortScan()){
	  ...
	  for(int ife=feStart; ife<feEnd && !getSTPixModuleGroup()->doAbortScan(); ife++){
		...
		getSTPixModuleGroup()->m_nSteps[i]=tmpScans[ife]->scanIndex(i);
	      ...
	      // call the main scan function in PixModuleGroup
		...
		int status;
		while ((status=getSTPixModuleGroup()->getPixController()->runStatus()==0) &&timeoutCnt<tMax){
		  PixLib::sleep(100);
		  timeoutCnt++;
		}
	      }
	  ...
	  do{
		...
		nloop = getSTPixModuleGroup() -> getPixController() -> nTrigger();
		...
		if(cfg.getSourceScanFlag()) {
		  ...
		  if(upc!=NULL) {
		    ...
		    getSTPixModuleGroup()->m_currSRAMReadoutReady = upc->getSRAMReadoutReady();
		    getSTPixModuleGroup()->m_currSRAMFull = upc->getSRAMFull();
		    getSTPixModuleGroup()->m_currMeasurementPause = upc->getMeasurementPause();
		    getSTPixModuleGroup()->m_currMeasurementRunning = upc->getMeasurementRunning();
		    getSTPixModuleGroup()->m_currSRAMFillLevel = upc->getSRAMFillLevel();
		    getSTPixModuleGroup()->m_currTriggerRate = upc->getTriggerRate();
		    getSTPixModuleGroup()->m_currEvtRate = upc->getEventRate();
			...
			{	
			    if(STEP_DEBUG)
				{
					...
					{
						qDebug() << "Board("<< upc->getBoardID() << ") SRAM_READOUT_AT: " <<cfg.getSramReadoutAt()<< "%, Currently at: " <<getSTPixModuleGroup()->m_currSRAMFillLevel<<"%";
					}
					debug_counter1++;	
				}
				...
				if(getSTPixModuleGroup()->m_currSRAMFull)
				{
					...
					//Send an event to the EUDAQProducer to display a warning message
       				eudaqScanStatusEvent* sse =  new eudaqScanStatusEvent(getSTPixModuleGroup() -> getPixController() -> getBoardID(), getSTPixModuleGroup()->m_currSRAMFull, getSTPixModuleGroup()->m_currSRAMFillLevel, getSTPixModuleGroup()->m_currTriggerRate);
		    		m_app->postEvent(getSTPixModuleGroup(), sse);
					STPixModuleGroup::global_readout =true;
				}

				//Check if SRAM is filled to desired read out
				if(getSTPixModuleGroup()->m_currSRAMFillLevel >= cfg.getSramReadoutAt() && !STPixModuleGroup::global_readout)
				{
					QMutexLocker locker(readoutFlagMutex);
				...
				//If this or some other thread (of a different board) triggered a read out, and our controller is not paused yet, pause and set read-out flag
	...
	if(upc->getSourceScanData(data, getSTPixModuleGroup()->m_readSram))
        {
          dataPendingEvent* dpe =  new dataPendingEvent(data, upc->getBoardID());
          ...
          getSTPixModuleGroup()->m_readSram = false;
			...
			{
			    if(getSTPixModuleGroup()->doForceRead()) 
				{
			    	upc->pauseRun();
			    	getSTPixModuleGroup()->m_forceRead = false;
			    	getSTPixModuleGroup()->m_readSram = true;
			    ...
			    if(upc->getSourceScanData(data, getSTPixModuleGroup()->m_readSram))
			    {
			    	if (getSTPixModuleGroup()->m_readSram) 
					{
						upc->resumeRun();
					}
					getSTPixModuleGroup()->m_readSram = false;
		...
		if( (nloop>0||upc!=0) && getSTPixModuleGroup()->doAbortScan() ){
		  pc->stopScan();
		  if(tmpScans[ife]->getSourceScanFlag()) getSTPixModuleGroup()->m_abortScan = false;
		}
		...
		if((m_checkScanCount%freq)==0 && upc==0){
	    ...
	    }while ((getSTPixModuleGroup()->getPixController()->runStatus()==1 && nAbortWait<100));
	      // end ROD scan loop
...
void STPixModuleGroup::processPixScanHistos(pixScanRunOptions scanOpts){ 
  ...
  if(scanOpts.writeToFile && !scanOpts.fileName.empty()){
	...
	if(scanOpts.writeToFile && !scanOpts.fileName.empty()){
	  ...
	  grpInq  = *(testInq->findRecord(getName()+"/PixModuleGroup"));
	  // new module entry
	  modInq  = createEmptyModule(grpInq, mname);
	  // need to close and re-open, otherwise FE subrecords not seen by Config::write...
