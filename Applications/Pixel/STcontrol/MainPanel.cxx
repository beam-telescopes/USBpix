...
void MainPanel::addDcs(ModListItem *mlitem){
  ...
  if(ndd.exec()){
	...
	if(crt->getGrpName(iGrp)==std::string(ndd.objectAssCtrlName->currentText().toLatin1().data())) {
      ...
      if(pGrp>=0 && ((ConfInt&)crt->getPixCtrlConf(pGrp)["general"]["AdapterCardFlavor"]).value()==1)
	genType += "BI"; 
      if(pGrp>=0 && ((ConfInt&)crt->getPixCtrlConf(pGrp)["general"]["AdapterCardFlavor"]).value()==2)
	genType += "Gpac"; 
...
void MainPanel::showInfo(){
  ...
  if (item!=0 && (item->getType()==ModListItem::tUSB || item->getType()==ModListItem::tROD)){
