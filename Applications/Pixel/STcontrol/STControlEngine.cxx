...
STControlEngine::STControlEngine( QApplication *app, STCLogContainer& log_in, QObject * parent) :
  ...
  conf["autoload"].addString("defPixScanName", m_psName, "",
			     "start-up loading of PixScan-file", true);
...
void STControlEngine::addCrateToDB(const char *crateName, std::vector<grpData> inGrpData, const char *DBfilename, 
				   IFtypes crtIFtype, bool addUsbDcs)
{
  PixConfDBInterface *myDB = 0;
  ...
  try{
    ...
    for(std::vector<grpData>::iterator IT=inGrpData.begin(); IT!=inGrpData.end(); IT++){
      ...
      if(crtIFtype==tUSBSys && addUsbDcs){
	// copy USBPixDcs entries, too - one per new group with ref. to USBPixController
// 	if(IT->myMods.size()>0 && IT->myMods[0].fname!=""){
// 	  PixConfDBInterface *dcsDB = DBEdtEngine::openFile(IT->myMods[0].fname.c_str());
// 	  DBInquire *dcsRoot = dcsDB->readRootRecord(1);
...
// 		std::string clname;
// 		dcsDB->DBProcess(*fit,READ, clname);
...
// 		  // call copy function for all depending fields
// 		  for(fit=(*dcsit)->fieldBegin(); fit!=(*dcsit)->fieldEnd(); fit++){
// 		    if((*fit)->getName()=="USBPixController"){
...
// 		    }else if((*fit)->getName()=="general_Index"){
// 		      DBField *fld = myDB->makeField("general_Index");
// 		      int myind = usbInd-1;
// 		      myDB->DBProcess(fld,COMMIT,myind);
// 		      new_inq->pushField(fld);
// 		      delete fld;
// 		      myDB->DBProcess(new_inq,COMMITREPLACE);
// 		    }else if((*fit)->getName()=="general_DeviceName"){
// 		      std::string ctrlName;
// 		      myDB->DBProcess(*fit,READ,ctrlName);
// 		      std::stringstream b;
// 		      b << IT->myROD.slot;
// 		      if(IT->myROD.slot>=0) ctrlName += " - board "+b.str();
// 		      DBField *fld = myDB->makeField("general_DeviceName");
// 		      myDB->DBProcess(fld,COMMIT,ctrlName);
// 		      new_inq->pushField(fld);
// 		      delete fld;
// 		      myDB->DBProcess(new_inq,COMMITREPLACE);
// 		    }else if((*fit)->getName()=="ActualClassName"){
// 		      std::string clName = "USBPixDcs";
// 		      if(IT->myROD.mode==1) clName = "USBBIPixDcs";
// 		      DBField *fld = myDB->makeField("ActualClassName");
...
// 		  }
// 		  // call copy function for all depending inquires
// 		  fit = (*dcsit)->findField("general_DeviceType");
// 		  std::string devType = "NONE";
// 		  if(fit!=(*dcsit)->fieldEnd())  myDB->DBProcess(*fit,READ,devType);
// 		  if(devType=="SUPPLY"){ // regulators have to be adapter board specific, so create from scratch
...
// 		      DBField *fld = myDB->makeField("general_ChannelDescr");
...
// 		      myDB->DBProcess(ch_inq,COMMITREPLACE);
// 		    }
// 		  } else{ // rest can be copied
// 		    for(recordIterator rit=(*dcsit)->recordBegin();rit!=(*dcsit)->recordEnd();rit++)
...
// 	  delete dcsDB;
// 	} else 
	if(IT->myMods.size()>0){// && (IT->myMods[0].assyType==1 || IT->myMods[0].assyType==2)){
	  ...
	  delete fld;
	  myDB->DBProcess(new_inq,COMMITREPLACE);
	  ctrlName = (IT->myROD.mode == 4)?"GPAC-Supplies":"USB-regulators";
	  if(IT->myROD.slot>=0)
	    ctrlName += "_"+b.str();
	  ...
	  myDB->DBProcess(new_inq,COMMITREPLACE);
	  std::string clName = "USBPixDcs";
	  if((IT->myROD.mode==1) || (IT->myROD.mode==2)) clName = "USBBIPixDcs";
	  if(IT->myROD.mode==4) clName = "USBGpacPixDcs";
	  ...
	  float nomvolts[4]={1.2f,1.5f,1.2f,1.5f};
	  float currLim = 1.0f;
	  if((IT->myROD.mode == 1) || (IT->myROD.mode == 2) || (IT->myROD.mode == 4)){
	    for(int i=0;i<4;i++){
	      std::stringstream a;
	      a << (i+1);
	      descr[i]=((IT->myROD.mode == 4)?"PWR":"CH")+a.str();
	      nomvolts[i]=(IT->myROD.mode == 4)?0.:2.0f;
	    }
	    // 2FE-system: need higher current limit
	    if(IT->myBOC.mode>=0) currLim = 1.5f;
	  }
	  for(int iCh=0;iCh<4;iCh++){
	    std::stringstream c;
	    c<<iCh;
	    ...
	    myDB->DBProcess(new_inq,COMMITREPLACE);
	    myDB->DBProcess(ch_inq,COMMIT);
	    fld = myDB->makeField((IT->myROD.mode == 4)?"general_GpacChannel":"general_ChannelDescr");
	    if(IT->myROD.mode == 4)
	      myDB->DBProcess(fld,COMMIT,iCh);
	    else if(IT->myMods[0].assyID==2 && IT->myROD.mode<1) // swap VDDA and VDDD for FE-I4B due to problem in SCC
	      myDB->DBProcess(fld,COMMIT,descrB[iCh]);
	    else
	      myDB->DBProcess(fld,COMMIT,descr[iCh]);
	    ch_inq->pushField(fld);
	    delete fld;
	  ...
	  if(IT->myROD.mode>1){
	    fld = myDB->makeField((IT->myROD.mode == 4)?"general_CurrentLimit":"general_CurrLim");
	  ...
	  if((IT->myROD.mode == 0) || (IT->myROD.mode == 3)){
	    // USBPix NTC ADC
	    std::stringstream d;
	    d << "_" << usbDcsInd;
	    usbDcsInd++;
...
void STControlEngine::init(const char *cfg_file, bool initRODs, const char *ps_file)
{
  ...
  if(ps_file!=0)
    pfname = ps_file;
  Config &my_cfg = getOptions();
  if(cfg_file==0 && ((ConfString&)my_cfg["paths"]["defCfgPath"]).value()!="." && 
     ((ConfString&)my_cfg["autoload"]["defCfgName"]).value()!="")
    cfname = ((ConfString&)my_cfg["paths"]["defCfgPath"]).value() + "/" + 
      ((ConfString&)my_cfg["autoload"]["defCfgName"]).value();
  if(ps_file==0 && ((ConfString&)my_cfg["paths"]["defCfgPath"]).value()!="." && 
     ((ConfString&)my_cfg["autoload"]["defPixScanName"]).value()!="")
    pfname = ((ConfString&)my_cfg["paths"]["defCfgPath"]).value() + "/" + 
      ((ConfString&)my_cfg["autoload"]["defPixScanName"]).value();
  if(cfname!=""){
    QApplication::setOverrideCursor(Qt::WaitCursor);
    loadDB(cfname.c_str());
...
void STControlEngine::processPrimList()
{
  ...
  case PrimListItem::DEF_SCAN:
    error = launchPrlScans((m_prlList[m_prlIndex]->getScanLabel()+(m_prlIndMode?(" "+a.str()):"")).c_str(),m_prlOutname.c_str(),
			   0, m_prlList[m_prlIndex]->getSubType(), m_prlList[m_prlIndex]->getReadDcs(), 
			   m_prlList[m_prlIndex]->getTimeout());
    break;
  case PrimListItem::CUST_SCAN:
    error = launchPrlScans((m_prlList[m_prlIndex]->getScanLabel()+(m_prlIndMode?(" "+a.str()):"")).c_str(),m_prlOutname.c_str(), 
			   m_prlList[m_prlIndex]->getPS(),0, m_prlList[m_prlIndex]->getReadDcs(), 
			   m_prlList[m_prlIndex]->getTimeout());
    break;
...
bool STControlEngine::launchPrlScans(const char *label, const char *fname, PixScan *cfg, int type, 
				     int readDcs, int timeToAbort)
{
  ...
  PixScan *myCfg = cfg;
  if(cfg==0)
    myCfg = new PixScan((PixLib::PixScan::ScanType)type);
