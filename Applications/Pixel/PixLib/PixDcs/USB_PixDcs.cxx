...
double USBPixDcsChan::ReadParam(std::string measType) {
  ...
  if(parent->m_USBPC->getUsbHandle()==0){
...
std::string USBPixDcsChan::ReadState(std::string){
  ...
  if(parent->m_USBPC->getUsbHandle()==0) return "unknown";
...
USBPixDcs::USBPixDcs(DBInquire *dbInquire, void *interface)
  ...
  m_USBPC = (USBPixController*)interface; //dynamic_cast<PixUSB>(interface);
...
void USBPixDcs::initHW(){
  ...
  SiUSBDevice *handle = m_USBPC->getUsbHandle();
  if(handle!=0){
