...
USBPixController::USBPixController(PixModuleGroup &modGrp) : 
	...
	for(unsigned int ib=0;ib<NBOARDS_MAX;ib++){
		...
		if(ib==0 ||  m_boardID[ib]>=0) {
			...
			if (m_BoardHandle[ib] != 0) {
				...
				if(UPC_DEBUG_GEN) cout<<"Detected adapter type="<< adapType<<endl;
				...
				if(m_AdapterCardFlavor!=adapType){
				  ...
				  m_MultiChipWithSingleBoard = (bool)(m_AdapterCardFlavor&0x1);
...
void USBPixController::writeScanConfig(PixScan &scn) { // write scan parameters
      ...
      it != m_chipIds.end(); it++)
  {
	...
	if(m_sourceScanFlag) {
		...
		m_USBpix->WriteRegister(CS_ENABLE_RJ45, (m_enableRJ45 ? 1 : 0));
		if(UPC_DEBUG_GEN) cout << "DEBUG USBPixCtrl: RJ45 " << (m_enableRJ45?"ON":"OFF") << endl;
		//} else {
		//  if(UPC_DEBUG_GEN) cout<<"DEBUG USBPixCtrl: RJ45 ON (beam test mode)" << endl;
    ...
    // to avoid race conditions:
          ...
          it != m_chipIds.end(); it++)
      {
        ...
        m_USBpix->ClearSRAM(*it);
			...
			if(m_scanConfigArray[7]&SHIFT_ENABLE) shiftPixMask(SHIFT_ENABLE, 1);
			if(m_scanConfigArray[7]&SHIFT_CAP0) shiftPixMask(0x40, 1);
			if(m_scanConfigArray[7]&SHIFT_CAP1) shiftPixMask(0x80, 1);
		}
	...
	if(m_upcScanBusy) {
		...
		if(m_scanBusy && !m_upcScanInit) { // normal scan
			...
			for (std::vector<int>::iterator it = m_chipIds.begin();
				...
				m_USBpix->GetSourceScanStatus(m_sramFull, measurementRunning, sramFillLevel, collectedTriggers, triggerRate, eventRate, *it);
				...
				if(!measurementRunning && !m_testBeamFlag /* && m_sramReadoutReady*/) { // TODO
					...
					m_USBpix->ClearSRAM(*it);
				} else if(measurementRunning  && !m_testBeamFlag && m_sramFull/* && m_sramReadoutReady*/) { // TODO
...
void USBPixController::shiftPixMask(int mask, int cap, int steps){
  ...
  if(m_USBpix!=0){
	...
	if(UPC_DEBUG_GEN) cout<<"DEBUG USBPixCtrl: shifting mask " << maskTags1[imask] << endl;
	//for(int ib=0;ib<NBOARDS_MAX;ib++){
        for (std::vector<int>::iterator it = m_chipIds.begin();
	     it != m_chipIds.end(); it++)
	  {
...
int USBPixController::detectAdapterCard(unsigned int boardId){
  ...
  for (unsigned int i = 0; i < nPages; i++){ // 64 byte page write
    status &= m_BoardHandle[boardId]->ReadI2C(CAL_EEPROM_ADD | 0x01, &data[dataPtr], CAL_EEPROM_PAGE_SIZE);
    dataPtr += CAL_EEPROM_PAGE_SIZE;
  }
  if (nBytes > 0){
    status &= m_BoardHandle[boardId]->ReadI2C(CAL_EEPROM_ADD | 0x01, &data[dataPtr], nBytes);
  }
  ...
  case CAL_DATA_HEADER_V2: // it is a BIC
    return 1;
  case CAL_DATA_HEADER_V1: // it is a SCA
    return 0;
  default:                 // either no board or GPAC - refine to distinguish
    return 2;
  }
}
