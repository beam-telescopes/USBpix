...
void RodPixController::sendCommandOutSync(Bits commands, int moduleMask, std::vector<Bits>& output) { //! Send command sync
}

void RodPixController::sendCommandOutAsync(Bits commands, int moduleMask, int commandID) { //! Send command async
}

bool RodPixController::checkOutReady(int commandID) {  //! Wait for command termination
  return false;
}

void RodPixController::getOutput(int commandID, std::vector<Bits>& output) {  //! Get command output
}
...
bool RodPixController::getSourceScanData(unsigned int * & , bool ){
  throw RodPixControllerExc(RodPixControllerExc::NOT_IMPLEMENTED, PixControllerExc::ERROR,  
			    getModGroup().getRodName()); 
  return false;
}
