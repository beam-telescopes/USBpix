  ...
  for (unsigned int i = 0; i < grp.m_param.size(); i++) {
    ...
    case ConfObj::STRING:
      addString(grp.m_param[i]->name(), grp.m_param[i]->valueString(), "", grp.m_param[i]->comment(), grp.m_param[i]->visible(), 
		((ConfString*)grp.m_param[i])->getType(), ((ConfString*)grp.m_param[i])->getExt());
      break;
    case ConfObj::VOID:
    default:
      break;
...
void ConfGroup::addString(std::string name, std::string &val, std::string def, std::string comm, bool vis, int type, std::string ext) {
  m_param.push_back(new ConfString(m_groupName+"_"+name, val, def, comm, vis, type, ext));
}
