...
//! PixFeConfig implementation for FE-I4B
...
//! Constructor
PixFeI4BConfig::PixFeI4BConfig(PixConfDBInterface *db, DBInquire *dbInquire, std::string name, int number) {
  // Members initialization
  m_db = db;
  m_dbInquire = dbInquire;
  m_name = name;
  m_className = "PixFeI4B";
...
//! Copy constructor
PixFeI4BConfig::PixFeI4BConfig(const PixFeConfig &c) {
  // Members initialization
  m_className = "PixFeI4B";

  // Try dynamic casting
  try {
    const PixFeI4BConfig& cfg = dynamic_cast<const PixFeI4BConfig&>(c);
...
//! Destructor
PixFeI4BConfig::~PixFeI4BConfig() {
}


//! Assignment operator
PixFeConfig& PixFeI4BConfig::operator = (const PixFeConfig& c) {
...
void PixFeI4BConfig::copyGlobReg(const PixFeConfig& source) {
  if(this != &source) {
    // Try dynamic casting
    try {
      const PixFeI4BConfig& cfg = dynamic_cast<const PixFeI4BConfig&>(source);
...
void PixFeI4BConfig::copyPixReg(const PixFeConfig& source, std::string name) {
  if(this != &source) {
    // Try dynamic casting
    try {
      const PixFeI4BConfig& cfg = dynamic_cast<const PixFeI4BConfig&>(source);
...
//! Trim copy method
void PixFeI4BConfig::copyTrim(const PixFeConfig& source, std::string name) {
  if(this != &source) {
    // Try dynamic casting
    try {
      const PixFeI4BConfig& cfg = dynamic_cast<const PixFeI4BConfig&>(source);
...
void PixFeI4BConfig::setupStructures() {
  ...
  if(m_structures.getGlobRegSize() == 0) {
    ...
    m_structures.setGlobReg("BufVgOpAmp",      8, 56);
    m_structures.setGlobReg("Reg6Spare",       8, 64);
    ...
    m_structures.setGlobReg("Reg9Spare",       8,112);
    ...
    m_structures.setGlobReg("GADCOpAmp",       8,200);
    ...
    m_structures.setGlobReg("Reg27Spare1",     1,405);
    m_structures.setGlobReg("GADC_Enable",     1,406);
    m_structures.setGlobReg("ShiftReadBack",   1,407);
    m_structures.setGlobReg("Reg27Spare2",     3,408);
    ...
    m_structures.setGlobReg("TempSensDiodeSel",2,448);
    m_structures.setGlobReg("TempSensDisable", 1,450);
    m_structures.setGlobReg("IleakRange",      1,451);
    m_structures.setGlobReg("Reg30Spare",     12,452);
    m_structures.setGlobReg("PlsrRiseUpTau",   3,464);
    m_structures.setGlobReg("PlsrPwr",         1,467);
    m_structures.setGlobReg("PlsrDelay",       6,468);
    m_structures.setGlobReg("ExtDigCalSW",     1,474);
    m_structures.setGlobReg("ExtAnaCalSW",     1,475);
    m_structures.setGlobReg("Reg31Spare",      1,476);
    m_structures.setGlobReg("GADCSel",         3,477);
    m_structures.setGlobReg("SELB0",          16,480);
    m_structures.setGlobReg("SELB1",          16,496);
    m_structures.setGlobReg("SELB2",           8,504);
    m_structures.setGlobReg("Reg34Spare1",     3,512);
    m_structures.setGlobReg("PrmpVbpMsnEn",    1,515);
    m_structures.setGlobReg("Reg34Spare2",     4,516);
    m_structures.setGlobReg("Chip_SN",        16,520);
    m_structures.setGlobReg("Reg1Spare",       7,536);
    m_structures.setGlobReg("SmallHitErase",   1,543);
    m_structures.setGlobReg("Eventlimit",      8,544);
...
void PixFeI4BConfig::setupData() {
  ...
  writeGlobRegister("Amp2Vbpf", 50);
  writeGlobRegister("Amp2VbpFol", 26);
  writeGlobRegister("BufVgOpAmp", 160);
  ...
  writeGlobRegister("Eventlimit", 0);
  writeGlobRegister("ExtAnaCalSW", 0);
  writeGlobRegister("ExtDigCalSW", 0);
  writeGlobRegister("FdacVbn", 30);
  writeGlobRegister("GADC_Enable", 0);
  writeGlobRegister("GADCOpAmp", 100);
  writeGlobRegister("GADCSel", 0);
  writeGlobRegister("GateHitOr", 0);
  writeGlobRegister("HitDiscCnfg", 2);
  writeGlobRegister("HITLD_In", 0);
  writeGlobRegister("IleakRange", 0); // check
  ...
  writeGlobRegister("PrmpVbpMsnEn", 0); // check
  ...
  writeGlobRegister("Reg30Spare", 0);
  writeGlobRegister("Reg31Spare", 0);
  writeGlobRegister("Reg34Spare1", 0);
  writeGlobRegister("Reg34Spare2", 0);
  writeGlobRegister("Reg6Spare", 0);
  writeGlobRegister("Reg9Spare", 0);
  writeGlobRegister("S0", 0);
  writeGlobRegister("S1", 0);
  writeGlobRegister("ShiftReadBack", 0);
  writeGlobRegister("SmallHitErase", 0);
  ...
  writeGlobRegister("VrefAnTune", 0);// TBD
  writeGlobRegister("VrefDigTune", 0);// TBD
  writeGlobRegister("Vthin_AltCoarse", 0);
  writeGlobRegister("Vthin_AltFine", 210);
