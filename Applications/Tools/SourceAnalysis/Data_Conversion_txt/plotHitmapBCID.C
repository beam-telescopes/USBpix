...
int plotHitmapBCID(const char * infilename = "out.txt", const char * outfilename = 0, bool loadStyle = true){
	ifstream myfile(infilename, ios::in);
  for (int ibcid = 0; ibcid<256; ibcid++){

    

    std::cout<<"BCID "<<ibcid<<std::endl;
    ...
    p3->AddText("TOT");
	  ...
	  p1->AddText("Hit map of the Atlas FE-I4, BCID "+TString(stringbcid));
	  p1->SetBorderSize(1);
	  p1->SetLineWidth(2);
	  p1->SetTextFont(42);
    ...
    char fileenum[3];
    sprintf(fileenum, "%03d", 255-ibcid);

    if (hasData) c1->SaveAs("analog_hitmap_"+TString(fileenum)+".pdf");
