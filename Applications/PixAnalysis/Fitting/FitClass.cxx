...
FitClass::FitClass()
{
  ...
  m_funcNames.insert(std::make_pair(16,"ToTfit_pol2"));
...
double FitClass::runFit(int npts, double *x, double *y, double *xerr, double *yerr, double *par, bool *fix_par, int funcID, double xmin, double xmax, bool show)
{
  ...
  if((vtype=runVirzi(funcID))>-1){
    ...
    if(show) for(i=0;i<npts;i++) printf("%lf %lf %lf %lf\n",x[i],y[i],xerr[i],yerr[i]);
...
bool FitClass::guessPar(int funcID, double *x, double *y, int npts, double *par)
{
  ...
  case 16:{ // parabolic fit
    ...
    x1 = x[istart];
    x2 = x[istart/2+(npts-1)/2];
    x3 = x[npts-1];
    y1 = y[istart];
    y2 = y[istart/2+(npts-1)/2];
    y3 = y[npts-1];
    par[2] = (y2-y3-(x2-x3)/(x1-x2)*(y1-y2))/(x1*x3+x2*x3-x1*x2-x3*x3);
    par[1] = (y1-y2)/(x1-x2)-par[2]*(x1+x2);
    par[0] = y2-par[1]*x2-par[2]*x2*x2;
    return true;}
