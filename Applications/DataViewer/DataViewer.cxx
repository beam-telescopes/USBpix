...
void DataViewer::processRightClick(const QPoint &){
  ...
  if(mac!=0 && mac->isHistoPlot()){
    ...
    act->setEnabled((pixScanLevel->value()<sclvl && pixScanLevel->value()>=0));
    modmenu.addSeparator();
    act = modmenu.addAction("Plot Q vs ToT", this, SLOT(plotQToTPlain()));
    act->setEnabled((pixScanLevel->value()<sclvl && pixScanLevel->value()>=0 && mac->getType()>=PixScan::TOT0 && mac->getType()<=PixScan::TOT13));
    act = modmenu.addAction("Plot Q vs ToT with fit function", this, SLOT(plotQToTFit()));
    act->setEnabled((pixScanLevel->value()<sclvl && pixScanLevel->value()>=0 && mac->getType()>=PixScan::TOT0 && mac->getType()<=PixScan::TOT13));
    modmenu.addSeparator();
    act = modmenu.addAction("Fit Q vs ToT on selected level", this, SLOT(fitQToT()));
    act->setEnabled((pixScanLevel->value()<sclvl && pixScanLevel->value()>=0 && mac->getType()>=PixScan::TOT0 && mac->getType()<=PixScan::TOT13));
    modmenu.addSeparator();
...
void DataViewer::plotQToT(bool doFit){
  m_lastQactID = doFit?5:4;
  m_pfact->ClearMemory();
  int scanpts[3]={Scanpt0->value(), Scanpt1->value(), Scanpt2->value()};
  int fid = -1;
  std::vector<int> pars;
  ...
  if(mac!=0 && mac->isHistoPlot()){
    ...
    QApplication::setOverrideCursor(Qt::WaitCursor);
    int retVal = m_pfact->plotQVsToT(mac->getData(), mac->getType(), pixScanLevel->value(), scanpts,  m_plotChip, 
				       ChipSel->value(), ColSel->value(), RowSel->value(), pars, fid);
    QApplication::restoreOverrideCursor();
    if(retVal<0) QMessageBox::warning(this,"DataView plot action","Error while plotting; code "+QString::number(retVal));
  }
...
void DataViewer::fitScan(bool QToTfit){
  m_lastQactID = 0;
  m_pfact->ClearMemory();
  //int scanpts[3]={Scanpt0->value(), Scanpt1->value(), Scanpt2->value()};
  ModuleAction *mac=dynamic_cast<ModuleAction*>(m_treeView->currentItem());
  if(mac!=0 && mac->isHistoPlot()){
	...
	if(fw->guessParBox->isChecked()){
      ...
      PixLib::PixScan::HistogramType errType = QToTfit?(PixLib::PixScan::HistogramType)mac->getType():PixLib::PixScan::MAX_HISTO_TYPES;
      if(fw->fromHistoBox->isChecked() && !QToTfit){
	PixLib::PixScan ps;
