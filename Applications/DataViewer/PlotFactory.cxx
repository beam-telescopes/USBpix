...
int PlotFactory::plotQVsToT(PixDBData &pdbd, int ps_type, int scan_level, int scanidx[3], int in_chip, 
			      int sel_chip, int sel_col, int sel_row, std::vector<int> pars, int fid){
  char line[1024];
  int binx,biny;
  int ptarr[4]={-1,-1,-1,0};
  ...
  if(ps_type<PixScan::TOT0 || ps_type>PixScan::TOT13) return -2;
  
  // map for top part of plot: use chi^2 if existing, otherwise one of the ToT histos
  int nCol, nRow, nFe;
  pdbd.getGeom(nRow, nCol, nFe);
  gROOT->cd();
  TH2F *tmpHi = 0;
  MATH2F *map = 0;
  pdbd.getGenericHisto(PixLib::PixScan::TOTCAL_CHI2, 0, scan_level+1);
  tmpHi = pdbd.getChipMap(in_chip);
  if(tmpHi==0){
    pdbd.getGenericHisto(PixLib::PixScan::TOT13, steps-1, scan_level);
    tmpHi = pdbd.getChipMap(in_chip);
  }
  if(tmpHi==0) return -2;
  map = new MATH2F(*((MATH2F*)tmpHi), true, m_dv);
  ...
  for(int i=0;i<ntot;i++){
    ...
    for(int j=0; j<steps; j++){
      ptarr[scan_level] = j;
      PixLib::Histo &TOTHisto = *(pdbd.getGenericPixLibHisto((PixLib::PixScan::HistogramType)(PixScan::TOT0+i), ptarr));
      vcal = (double)scanpt[j];
      double charge = Cval*(vcalpar[0]+vcalpar[1]*vcal+vcalpar[2]*vcal*vcal+vcalpar[3]*vcal*vcal*vcal);
      chg_sumsqr += TOTHisto(binx-1, biny-1) * charge * charge;
      chg_sum += TOTHisto(binx-1, biny-1) * charge;
      hitsum += TOTHisto(binx-1, biny-1);
    }
    if(hitsum > 2) {
      chg_sumsqr /= hitsum;
      yval[i] = chg_sum / hitsum;
      yerr[i] = sqrt((chg_sumsqr - yval[i]*yval[i])/(hitsum-1.0));
      if(yerr[i]<dchge) yerr[i] = dchge; // should not have errors smaller than half a VCAL step
    }else{
      yval[i] = 0.;
      yerr[i] = 0.;
    }
  ...
  gROOT->cd();
  TGraphErrors *gr = new TGraphErrors(ntot,xval,yval, xerr, yerr);
  // graph general titel
  std::stringstream strs;
  strs << "Q vs. ToT, Chip " << sel_chip << " col " << sel_col << " row " << sel_row;
  gr->SetTitle(strs.str().c_str());

  gr->GetXaxis()->SetTitle("ToT [BCU]");
  ...
  TF1 *fun=0;
  // if function plotting was requested, prepare that
  if(fid>=0){
    delete m_RDBfunc;
    int npar = m_fc->getNPar(fid);
    if(m_fc->hasVCAL(fid)) // allow special VCAL fit
      npar += 4;
    fun = new TF1(m_fc->getFuncName(fid).c_str(), (double (*)(double *, double *))m_fc->getFunc(fid), 
		  xval[0], xval[ntot-1], npar);
    m_RDBfunc = fun; // will allow us to delete this later
    std::vector<std::string> labels = m_fc->getParNames(fid);
    for(int pi=0;pi<m_fc->getNPar(fid);pi++){
      double parVal = 0;
      if(pars[pi]<PixLib::PixScan::MAX_HISTO_TYPES){
	TH2F *tmpHi = (TH2F*)pdbd.getGenericHisto((PixLib::PixScan::HistogramType)pars[pi], -1, scan_level+1);
	pdbd.PixCCRtoBins(sel_chip, sel_col, sel_row, binx, biny);
	parVal = (tmpHi==0)?0:(double)tmpHi->GetBinContent(binx,biny);
      } else
	parVal = (double)pdbd.getNevents();
    ...
    if(m_fc->hasVCAL(fid)){ // allow special VCAL fit
      for(int j=0;j<4;j++){
	fun->SetParameter(j+m_fc->getNPar(fid),vcalpar[j]);
	std::stringstream a;
	a << j;
	fun->SetParName(j+m_fc->getNPar(fid),("VCAL par "+a.str()).c_str());
      }
