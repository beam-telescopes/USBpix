...
FitWin::FitWin(PixDBData &data, FitClass &fc, QWidget* parent, Qt::WindowFlags fl, int htype, int loopLev)
  ...
  int iErf=0, iToT=0, iToTnew=0, iLin=0;
  for(int i=0;i<100 && m_fitClass.getNPar(i)>0;i++){
    ...
    if(fname=="ToT-calibration FE-I4 Polynomial 2nd order") iToTnew = i;
  }
  ...
  if(htype>=0 && loopLev>=0 && loopLev<3){
    ...
    case PixLib::PixScan::TOT_MEAN:
      ...
      break;
    case PixLib::PixScan::TOT0:
    case PixLib::PixScan::TOT1:
    case PixLib::PixScan::TOT2:
    case PixLib::PixScan::TOT3:
    case PixLib::PixScan::TOT4:
    case PixLib::PixScan::TOT5:
    case PixLib::PixScan::TOT6:
    case PixLib::PixScan::TOT7:
    case PixLib::PixScan::TOT8:
    case PixLib::PixScan::TOT9:
    case PixLib::PixScan::TOT10:
    case PixLib::PixScan::TOT11:
    case PixLib::PixScan::TOT12:
      FitType->setCurrentIndex(iToTnew);
      fitfunSel(iToTnew);
      ErrorBox->hide();
      break;
    default:
...
void FitWin::fitfunSel(int funID){
  ...
  for(int i=0;i<npar;i++){
    ...
    m_parValues[i]->show();
    m_parFixed[i]->show();
    m_parValues[i]->setEnabled(!guessParBox->isChecked());
    m_parFixed[i]->setEnabled(!guessParBox->isChecked());
