  ...
  : QDialog(parent,fl), m_engine(in_engine)
{
  ...
  // load default scans
  PixLib::PixScan *ps = new PixLib::PixScan(PixLib::PixScan::DIGITAL_TEST, PixLib::PixModule::PM_FE_I4A);
...
void PrlItemInit::setType(int type)
{
  if(type==PrimListItem::CUST_SCAN){
    prliCustType->show();
    custLabel->show();
    flavourI3Button->show();
    flavourI4Button->show();
  } else{
    prliCustType->hide();
    custLabel->hide();
    flavourI3Button->hide();
    flavourI4Button->hide();
  }
}
