...
void ConfigCreatorHelper::readModuleInfo(std::string fname, std::string mDecName, std::string &mccFlv, std::string &feFlv, int &nFe, int &nFeRows, std::string &ccpdFlv){
  PixLib::PixConfDBInterface * confDBInterface = DBEdtEngine::openFile(fname.c_str(), false); 
  PixLib::DBInquire *root = confDBInterface->readRootRecord(1);
...
//     feFlv = (int)mod.getFEFlavour();
    ...
    if(mod.pixCCPD()!=0 && mod.pixCCPD()->config()["ClassInfo"].name()!="__TrashConfGroup__" &&
      ...
      ccpdFlv = ((ConfString&)mod.pixCCPD()->config()["ClassInfo"]["ClassName"]).value();
    }
  } else
...
PixLib::PixModuleGroup* ConfigCreatorHelper::createPixModGrp(std::string name, std::string ctrlType, std::vector<std::string> mnames, 
							     std::string feType, int nFe, int nFeRows, std::string mccType, std::string ccpdype, int ctrlOpt,
							     std::map<int, std::vector<std::string> > dbFnames, 
							     std::map<int, std::vector<std::string> > dbMnames){

  PixLib::PixModuleGroup *pmg = new PixLib::PixModuleGroup(name, ctrlType, mnames, feType, nFe, nFeRows, mccType, ccpdype, ctrlOpt);
