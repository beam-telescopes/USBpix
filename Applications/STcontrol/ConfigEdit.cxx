...
void ConfigEdit::StoreCfg(){
  ...
  bool autoFix = false;
  for (int Chip = 0; Chip < (m_nFEs-1); Chip++){
    for (int Chip2 = Chip+1; Chip2 < m_nFEs; Chip2++){
...
void ConfigEdit::PTableClicked(int row, int col){
  ...
  if(QApplication::mouseButtons()==Qt::RightButton){ // only process right clicks
    if(col>0){ 
      ...
      if(m_nFEs>1) tablemenu.addAction("Load file (all chips)",this, SLOT(LoadPTableFileAll()));
...
void ConfigEdit::GTableClicked(int row, int col){
  ...
  if(QApplication::mouseButtons()==Qt::RightButton){ // only process right clicks
	...
	GlobalTable->item(row,col)->setText(QString::number(getval.SpinBox->value()));
      ...
      if(m_nFEs>1) tablemenu.addAction("Set entire column",           this,  SLOT(setValueAll()));
...
void ConfigEdit::CopyMap(bool all, const char *regName){
  if(m_currCol<3 || m_currRow<0 || m_currCol>=PixelTable->columnCount() || m_currRow>=PixelTable->rowCount()) return;

  // select mask from which to copy
  std::string objName = PixelTable->horizontalHeaderItem(m_currCol)->text().toLatin1().data();
  RefWin mlist(this);
  ...
  Config &feconf = *(m_feConf[m_currRow]);
  Config &tmppr = feconf.subConfig("PixelRegister_0/PixelRegister");
  ...
  std::string prname = "";
  if(regName!=0) prname = regName;
  else if(mlist.FuncSel->count()>0){
    if(mlist.exec()){
      prname = (mlist.FuncSel->currentText()).toLatin1().data();
    }
  }
  if(prname!=""){
    int rmin = m_currRow, rmax=m_currRow+1;
...
void ConfigEdit::InvertMap(bool all){
  if(m_currCol<3 || m_currRow<0 || m_currCol>=PixelTable->columnCount() || m_currRow>=PixelTable->rowCount()) return;
