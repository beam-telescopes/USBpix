...
TXilinxChip::TXilinxChip(bool m_is_fx)
{
	is_fx = m_is_fx;
...
TXilinxChip::ReadBitStream(const char *filename)
{
	...
	if (SF_DEBUG_USB>0) printf("***> TXilinxChip::ReadBitStream(%s)\n",filename);
	...
	*(time.index) = 'd';
	...
	if (fread(filepreamble, 1, sizeof(filepreamble), bitfile) != sizeof(filepreamble))
	{
#ifndef AVOID_MSG_BOX
	...
	*/
	...
	if (SF_DEBUG_USB>3) printf("***> TXilinxChip::ReadBitStream() %c %c %c %c :%s :%s :%s :%s \n"
		,*name.index,*type.index,*date.index,*time.index
		,name.data,type.data,date.data,time.data);
	if (SF_DEBUG_USB>3) printf("***> TXilinxChip::ReadBitStream()(next is data preamble) \n");
	...
	if (SF_DEBUG_USB>3) printf("***> TXilinxChip::ReadBitStream() MODE= N=%d E=%d M=%d \n",
		isNormalMode,isExpressMode,isSelectMapMode);
	...
	if (isExpressMode) {
		...
		size_t rs = 0;
		rs += fread(&(bitstream.dummy)       , 1, 1, bitfile);
		rs += fread(&(bitstream.dummy)       , 1, 1, bitfile);
		rs += fread(&(bitstream.dummy)       , 1, 1, bitfile);
		rs += fread(&(bitstream.lengthcount2), 1, 1, bitfile);
		rs += fread(&(bitstream.lengthcount1), 1, 1, bitfile);
		rs += fread(&(bitstream.lengthcount0), 1, 1, bitfile);
		...
		if (isNormalMode) {
			...
			size_t rs = 0;
			rs += fread(&(bitstream.dummy)       , 1, 1, bitfile);
			rs += fread(&(bitstream.lengthcount3), 1, 1, bitfile);
			rs += fread(&(bitstream.lengthcount2), 1, 1, bitfile);
			rs += fread(&(bitstream.lengthcount1), 1, 1, bitfile);
			rs += fread(&(bitstream.lengthcount0), 1, 1, bitfile);
			// calculate length count (number of *bits*)
			...
			if (isSelectMapMode) {
				...
				nBytes = fread(bitstream.data, 1, bitstream.size, bitfile);
				fclose(bitfile);
				if (SF_DEBUG_USB>3) printf("***> TXilinxChip::ReadBitStream() Nbytes=%d of %d\n", 
						nBytes,bitstream.size);
...
TXilinxChip::DownloadXilinx(std::string filename)
{
	...
	if (SF_DEBUG_USB>3) printf("***> TXilinxChip::DownloadXilinx() ReadBitStream() done, %d bytes \n",bitstream.size);
	...
	if (is_fx) {
		...
		if (SF_DEBUG_USB>3) printf("***> TXilinxChip::DownloadXilinx() WriteXilinxConfData() FX done..\n");
		...
		if (SF_DEBUG_USB>3) printf("***> TXilinxChip::DownloadXilinx() WriteXilinxConfData() NON-FX ..\n");
	...
	if (SF_DEBUG_USB>3) printf("***> TXilinxChip::DownloadXilinx() GetXilinxConfPin(%d)....\n",xp_done);

	/* check if device has become active */
	if (GetXilinxConfPin(xp_done) != 1) {
		if (SF_DEBUG_USB>3) printf("***> TXilinxChip::DownloadXilinx() GetXilinxConfPin(%d)!=1 !!!\n",xp_done);

		free (bitstream.data);
		return false;
	}

	if (SF_DEBUG_USB>3) printf("***> TXilinxChip::DownloadXilinx() ALL done !!!\n");
