...
CDataFileI3::CDataFileI3(t_Str szFileName)
{
	m_bDirty = false;
...
// SetFileName
// Set's the m_szFileName member variable. For use when creating the CDataFileI3
// object by hand (-vs- loading it from a file
void CDataFileI3::SetFileName(t_Str szFileName)
{
	if (m_szFileName.size() != 0 && CompareNoCase(szFileName, m_szFileName) != 0)
	{
		m_bDirty = true;

		Report(E_WARN, "[CDataFileI3::SetFileName] The filename has changed from <%s> to <%s>.",
...
bool CDataFileI3::Load(t_Str szFileName)
{
...
//	fstream File(szFileName.c_str(), ios::in|ios::nocreate);
	...
	{
		Report(E_INFO, "[CDataFileI3::Load] Unable to open file. Does it exist?");
		return false;
	}
...
// was never called (the CDataFileI3 object was created manually), then you
// must set the m_szFileName variable before calling save.
bool CDataFileI3::Save()
{
	if ( KeyCount() == 0 && SectionCount() == 0 )
	{
		// no point in saving
		Report(E_INFO, "[CDataFileI3::Save] Nothing to save.");
		return false; 
	}

	if ( m_szFileName.size() == 0 )
	{
		Report(E_ERROR, "[CDataFileI3::Save] No filename has been set.");
		return false;
	}
	...
	{
		Report(E_ERROR, "[CDataFileI3::Save] Unable to save file.");
		return false;
	}
...
bool CDataFileI3::SetKeyComment(t_Str szKey, t_Str szComment, t_Str szSection)
{
	KeyItor k_pos;
	t_Section* pSection;
...
// was not found.
bool CDataFileI3::SetSectionComment(t_Str szSection, t_Str szComment)
{
	SectionItor s_pos;
...
bool CDataFileI3::SetValue(t_Str szKey, t_Str szValue, t_Str szComment, t_Str szSection)
{
...
bool CDataFileI3::SetFloat(t_Str szKey, double fValue, t_Str szComment, t_Str szSection)
{
	char szStr[64];
...
bool CDataFileI3::SetInt(t_Str szKey, int nValue, t_Str szComment, t_Str szSection)
{
	char szStr[64];
...
bool CDataFileI3::SetBool(t_Str szKey, bool bValue, t_Str szComment, t_Str szSection)
{
...
t_Str CDataFileI3::GetValue(t_Str szKey, t_Str szSection) 
{
	t_Key* pKey = GetKey(szKey, szSection);
...
// Returns the key value as a t_Str object. A return value of
// t_Str("") indicates that the key could not be found.
t_Str CDataFileI3::GetString(t_Str szKey, t_Str szSection)
{
...
// not found.
double CDataFileI3::GetFloat(t_Str szKey, t_Str szSection, double def_val)
{
	...
	return (double)atof( szValue.c_str() );
}

// GetInt
// Returns the key value as an integer type. Returns INT_MIN if the key is
// not found.
int	CDataFileI3::GetInt(t_Str szKey, t_Str szSection, int def_val)
{
	t_Str szValue = GetValue(szKey, szSection);
...
// not found.
bool CDataFileI3::GetBool(t_Str szKey, t_Str szSection, bool def_val)
{
	bool bValue = false;
	t_Str szValue = GetValue(szKey, szSection);
...
bool CDataFileI3::DeleteSection(t_Str szSection)
{
	SectionItor s_pos;
...
bool CDataFileI3::DeleteKey(t_Str szKey, t_Str szFromSection)
{
	KeyItor k_pos;
	t_Section* pSection;
...
// the new value. If it does not locate the key, it will create a new key with
// the proper value and place it in the section requested.
bool CDataFileI3::CreateKey(t_Str szKey, t_Str szValue, t_Str szComment, t_Str szSection)
{
	bool bAutoKey = (m_Flags & AUTOCREATE_KEYS) == AUTOCREATE_KEYS;
	bool bReturn  = false;
...
bool CDataFileI3::CreateSection(t_Str szSection, t_Str szComment)
{
	t_Section* pSection = GetSection(szSection);

	if ( pSection )
	{
		Report(E_INFO, "[CDataFileI3::CreateSection] Section <%s> allready exists. Aborting.", szSection.c_str());
		return false;
	}
...
bool CDataFileI3::CreateSection(t_Str szSection, t_Str szComment, KeyList Keys)
{
...
t_Key*	CDataFileI3::GetKey(t_Str szKey, t_Str szSection)
{
	KeyItor k_pos;
	t_Section* pSection;
...
t_Section* CDataFileI3::GetSection(t_Str szSection)
{
	SectionItor s_pos;
...
t_Str CDataFileI3::CommentStr(t_Str szComment)
{
...
t_Str CDataFileI3::GetNextWord(t_Str& CommandLine)
{
...
// match.
int CDataFileI3::CompareNoCase(t_Str str1, t_Str str2)
{
#ifdef __VISUALC__
...
void CDataFileI3::Trim(t_Str& szStr)
{
...
// bytes written.
int CDataFileI3::WriteLn(fstream& stream, const char *fmt, ...)
{
...
void CDataFileI3::Report(e_DebugLevel DebugLevel,const char *fmt, ...)
{
	char buf[MAX_BUFFER_LEN];
	int nLength;
	t_Str szMsg;
