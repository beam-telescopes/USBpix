...
ConfigRegisterI3::ConfigRegisterI3(SiUSBDevice * Handle) // myUSB(in_hUSB)
{
	myUSB = Handle;
	SRAMdata = new unsigned char [SRAM_BYTESIZE]; // write buffer
	SRAMdataRB = new unsigned char [SRAM_BYTESIZE]; // read buffer
	SRAMwordsRB = new unsigned int [SRAM_BYTESIZE/4]; // read buffer, 32-bit words
...
{
	if ((StartAdd >= 0) && (StartAdd < SRAM_BYTESIZE) && (StartAdd%4 == 0) && (NumberOfWords > 0) && (NumberOfWords <= ((SRAM_BYTESIZE/4) - (StartAdd/4))))
	{
		// set address for high-speed interface
		...
		// high-speed read
		myUSB->ReadBlock(SRAMdataRB, (NumberOfWords * 4));
...
void ConfigRegisterI3::WriteSRAM(int StartAdd, int NumberOfWords)
{
	if ((StartAdd >= 0) && (StartAdd < SRAM_BYTESIZE) && (StartAdd%4 == 0) && (NumberOfWords > 0) && (NumberOfWords <= ((SRAM_BYTESIZE/4) - (StartAdd/4))))
	{
		...
		myUSB->WriteBlock(SRAMdata, (NumberOfWords * 4));
	}
}
...
{
	for(int i = 0, byte = 0; i < (SRAM_BYTESIZE/4); i++, byte+=4)
	{
...
bool ConfigRegisterI3::WriteSRAMWords(char *filename)
{
	...
	for (int i = 0; i < (SRAM_BYTESIZE/4); i++)
	{
...
bool ConfigRegisterI3::WriteTurboDAQFromRawData(const char * filename, bool new_file = true, bool close_file = true)
{
	...
	if (new_file == false) // append data to existing file
	{
			...
			for (int j = bcid_window_start; j < bcid_window_end; j++)
			{
				...
				if ((TRIGGER_WORD_HEADER_MACRO(SRAMwordsRB[j]) == 0) && (HEADER_MACRO(SRAMwordsRB[j]) == 1) && ((FLAG_MACRO(SRAMwordsRB[j]) & FLAG_WO_STATUS) == FLAG_WO_STATUS)) // EOE word
				{
					...
					if (FLAG_MACRO(SRAMwordsRB[j]) != FLAG_NO_ERROR)
					{
						...
						if ((FLAG_MACRO(SRAMwordsRB[j]) & FLAG_ERROR_4) == FLAG_ERROR_4) fout << "ERROR: ERROR 4" << endl; // ERROR 4, Command/Global Register Parity Error
						if ((FLAG_MACRO(SRAMwordsRB[j]) & FLAG_ERROR_8) == FLAG_ERROR_8) fout << "ERROR: ERROR 8" << endl; // ERROR 8, HitParity Error
					}
	...
	if (close_file == true)
	{
		...
		fout << endl << "# " << "Valid BCID Windows:";
		fout.flags(std::ios::right);
		fout.width(10);
		fout << total_valid_bcid_window;
		fout << endl << "# " << "BCID Windows w/o Hits:";
		fout.flags(std::ios::right);
		fout.width(7);
...
*/
void ConfigRegisterI3::GetuCScanStatus(int & MaskStep, bool & ScanBusy, bool & ScanReady, bool & ScanCanceled, bool & EOEerror)
{
	unsigned char Data[4];
	myUSB->ReadRegister(Data);
	myUSB->ReadRegister(Data); // call it twice to get recent data
	MaskStep = (int(Data[1]) << 8) | int(Data[0]);
	EOEerror = bool(0x01 & Data[2]);
	ScanBusy = bool(0x01 & (Data[2] >> 1));
	ScanReady = bool(0x01 & (Data[2] >> 2));
	ScanCanceled = bool(0x01 & (Data[2] >> 3));
}
...
* SRAMFillingLevel: percentage of SRAM filling level
* NumberOfTriggers: current number of LV1 triggers
* NumberOfHitWords: current number of hit words
* NumberOfEOEWords: current number of EOE words
*/
void ConfigRegisterI3::GetSourceScanStatus(bool & SRAMReadoutReadySignal, bool & SRAMFullSignal, bool & MeasurementPauseSignal, bool & MeasurementRunning, int & SRAMFillingLevel, int & NumberOfTriggers, int & NumberOfHitWords, int & NumberOfEOEWords)
{
	...
	MeasurementRunning = (ReadRegister(CS_MEASUREMENT_START_STOP) > 0) ? true : false;
	SRAMFillingLevel = (ReadRegister(CS_SRAM_ADD_FSM_20_13)*100)/255;
	...
	NumberOfEOEWords = (ReadRegister(CS_EOE_WORD_COUNTER_3) << 24) | (ReadRegister(CS_EOE_WORD_COUNTER_2) << 16) | (ReadRegister(CS_EOE_WORD_COUNTER_1) << 8) | ReadRegister(CS_EOE_WORD_COUNTER_0);
}
