...
public:
	...
	void GetuCScanStatus(int & MaskStep, bool & ScanBusy, bool & ScanReady, bool & ScanCanceled, bool & EOEerror);
	void GetSourceScanStatus(bool & SRAMReadoutReadySignal, bool & SRAMFullSignal, bool & MeasurementPauseSignal, bool & MeasurementRunning, int & SRAMFillingLevel, int & NumberOfTriggers, int & NumberOfHitWords, int & NumberOfEOEWords);

	unsigned int * SRAMwordsRB;
private:
	SiUSBDevice * myUSB;
	void BuildWords();
