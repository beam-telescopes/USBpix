...
// Trigger Mode
#define CS_TRIGGER_MODE					26 // 2-bit register, select external trigger mode: 2'b00: no external trigger, uC scan / strobe signal; 2'b01: external trigger / hitbus / TLU no handshake; 2'b10: TLU simple handshake; 2'b11: TLU trigger data handshake
// Source Scan
...
#define CS_EOE_HIT_WORD_TIME_OUT		30 // 8-bit register, time out for receiving EoE words or hit words in number of BC, 0 means off
#define CS_COUNTER_MODE					31 // 2-bit register, select 2'b00: triggers, 2'b01: hits words, 2'b10: EOE words, 2'b11: spare
#define CS_NUMBER_EVENTS_0				32 // 32-bit register, set number of LV1 triggers / hits / EOE to measure, see also CS_COUNTER_MODE, 0 means off
#define CS_NUMBER_EVENTS_1				33
#define CS_NUMBER_EVENTS_2				34
#define CS_NUMBER_EVENTS_3				35
#define CS_TRIGGER_COUNTER_0			36 // 32-bit register, counting triggers
#define CS_TRIGGER_COUNTER_1			37
#define CS_TRIGGER_COUNTER_2			38
#define CS_TRIGGER_COUNTER_3			39
#define CS_HIT_WORD_COUNTER_0			40 // 32-bit register, counting hit words
#define CS_HIT_WORD_COUNTER_1			41
#define CS_HIT_WORD_COUNTER_2			42
#define CS_HIT_WORD_COUNTER_3			43
#define CS_EOE_WORD_COUNTER_0			44 // 32-bit register, counting EOE words
#define CS_EOE_WORD_COUNTER_1			45
#define CS_EOE_WORD_COUNTER_2			46
#define CS_EOE_WORD_COUNTER_3			47
// TLU
#define CS_ENABLE_RJ45					48 // 1-bit register, enable RJ45 port, if nothing is connected input wires are tied to high
#define CS_SAVE_COMPLETE_TRIGGER_NUMBER	49 // 1-bit register, save complete trigger number (31 bit instead of 23 bit)
#define CS_TLU_TRIGGER_DATA_MSB_FIRST	50 // 1-bit register, enable to flip bit order of trigger number
#define CS_TLU_TRIGGER_DATA_LENGTH		51 // 8-bit register, length of TLU trigger number in bits, max. 32-bit
#define CS_TLU_TRIGGER_DATA_DELAY		52 // 8-bit register, set length of additional wait cycles before saving TLU trigger data from shift register (depends on TLU trigger data length: 32 - CS_TLU_TRIGGER_DATA_LENGTH + additional delay = CS_TLU_TRIGGER_DATA_DELAY)
#define CS_TLU_TRIGGER_LOW_TIME_OUT		53 // 8-bit register, set time out for TLU trigger not going low; stops source scan if time out is over; set to a number smaller than CS_DELAY_LV1; 0 means off
// Noise Occupancy
...
#define EOE_WORD_ERROR				0x01
#define SCAN_BUSY					0x02
#define SCAN_READY					0x04
