...
#else
  strncpy(CalData.name, name, std::min(MAX_CHANNEL_NAME_SIZE, (int)strlen(name)));
#endif
...
{
	unsigned short raw = (unsigned short)mA_value * CURRENT_LIMIT_GAIN;  
...
bool HL_GPAC::ReadCalDataFile(const char * adcfilename)
{
	...
	{ 
		...
		CH[i]->CalData.DefaultValue = IniFile->GetFloat("Default value", ss.str().c_str(), (float)CH[i]->CalData.DefaultValue);
		CH[i]->CalData.DACOffset  = IniFile->GetFloat("DAC offset", ss.str().c_str(), (float)CH[i]->CalData.DACOffset);
		CH[i]->CalData.DACGain    = IniFile->GetFloat("DAC gain",   ss.str().c_str(), (float)CH[i]->CalData.DACGain);
		CH[i]->CalData.VADCOffset = IniFile->GetFloat("Voltage sense offset", ss.str().c_str(), (float)CH[i]->CalData.VADCOffset);
		CH[i]->CalData.VADCGain   = IniFile->GetFloat("Voltage sense gain",   ss.str().c_str(), (float)CH[i]->CalData.VADCGain);
		CH[i]->CalData.IADCOffset = IniFile->GetFloat("Current sense offset", ss.str().c_str(), (float)CH[i]->CalData.IADCOffset);
		CH[i]->CalData.IADCGain   = IniFile->GetFloat("Current sense gain",   ss.str().c_str(), (float)CH[i]->CalData.IADCGain);
	}
	return status;
}
...
bool HL_GPAC::WriteCalDataFile(const char * adcfilename)
{
	...
	{ 
		ss.str("");
		ss << CH[i]->CalData.name << " calibration constants";
		IniFile->SetFloat("Default voltage", (float)CH[i]->CalData.DefaultValue, "Default value", ss.str().c_str());
		IniFile->SetFloat("DAC offset", (float)CH[i]->CalData.DACOffset, "DAC offset (counts)", ss.str().c_str());
		IniFile->SetFloat("DAC gain",   (float)CH[i]->CalData.DACGain ,  "DAC gain (counts)",   ss.str().c_str());
		IniFile->SetFloat("Voltage sense offset", (float)CH[i]->CalData.VADCOffset, "VADC offset (counts)", ss.str().c_str());
		IniFile->SetFloat("Voltage sense gain",   (float)CH[i]->CalData.VADCGain ,  "VADC gain (counts/V)",   ss.str().c_str());
		IniFile->SetFloat("Current sense offset", (float)CH[i]->CalData.IADCOffset, "CADC offset (counts)", ss.str().c_str());
		IniFile->SetFloat("Current sense gain",   (float)CH[i]->CalData.IADCGain ,  "CADC gain (counts/A)",   ss.str().c_str());
	}
	return IniFile->Save();
}
